...
const EditPage = () => {
  ...
  useEffect(() => {
    if (data) {
      ...
      setCreatorsState(data.createdBy);
      setDateState(data.releasedDate);
      setCoverState(data.photoURL);
      setBgState(data.bgPhotoURL);
      setEpisodeState(data.numOfEpisodes);
      setRatingState(data.rating);
      setDescriptionState(data.descriptionState);
    }
  }, [data]);
  ...
  const handleUpdate = (e) => {
    ...
    setIsLoading(true);
    axios
      .put(`http://localhost:14602/api/Shows/${id}`, {
        title: titleState,
  ...
  return (
    <Layout>
      ...
      <form onSubmit={handleUpdate} className="mt-4 mb-32 space-y-6">
        <InputLabel
          state={titleState}
