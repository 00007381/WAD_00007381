  ...
  return (
    ...
    >
      <Layout>
        ...
        <LisOfCharacters characters={data.characters.$values} />
      </Layout>
    </div>
  );
};
...
const LisOfCharacters = ({ characters }) => {
  return (
    <div className="bg-white shadow-xl p-4 mb-6 rounded-lg">
      <h1 className="text-2xl mb-4 font-semibold">List of Characters:</h1>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-y-10  justify-items-auto gap-6 ">
        {characters &&
          characters.map((item) => {
            return <ShowCard show={item} key={item.index} />;
          })}
      </div>
    </div>
  );
